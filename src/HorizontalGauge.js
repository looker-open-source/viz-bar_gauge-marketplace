...
const drawHorizontal = props => {
  ...
  svg
    ...
    .attr(
      'viewBox',
      `${props.w / -2} ${props.h / -2} ${props.w} ${props.h * 0.9}`
    );
  ...
  svg
    ...
    .attr(
      'height',
      d3.select('.horizontal-gauge').attr('height') * (props.arm / 100 + 1)
    )
    ...
    .attr(
      'y',
      d3.select('.horizontal-gauge').node().getBBox().y -
        (d3.select('.left-arm').node().getBBox().height -
          d3.select('.horizontal-gauge').node().getBBox().height)
    )
    .attr('x', props.w / -2);
  svg
    .append('rect')
    ...
    .attr(
      'y',
      d3.select('.horizontal-gauge').node().getBBox().y -
        (d3.select('.right-arm').node().getBBox().height -
          d3.select('.horizontal-gauge').node().getBBox().height)
    );
  ...
  svg
    ...
    .attr(
      'x',
      0 +
        d3.select('.horizontal-gauge').node().getBBox().width / 2 -
        d3.select('.left-arm').node().getBBox().width -
        d3.select('.max-label').node().getBBox().width
    )
    .attr(
      'y',
      d3.select('.right-arm').node().getBBox().y -
        d3.select('.max-label').node().getBBox().height
    );
  svg
    .append('text')
    ...
    .attr(
      'y',
      d3.select('.left-arm').node().getBBox().y -
        d3.select('.min-label').node().getBBox().height
    );
  ...
  svg
    ...
    .attr(
      ...
      `${
        proportion *
        (d3.select('.horizontal-gauge').node().getBBox().width -
          d3.select('.left-arm').node().getBBox().width * 2)
      }`
    )
  ...
  svg
    ...
    .attr(
      'height',
      d3.select('.horizontal-gauge').attr('height') * (props.spinner / 400 + 1)
    )
    ...
    .attr(
      'x',
      d3.select('.horizontal-fill').node().getBBox().x +
        d3.select('.horizontal-fill').node().getBBox().width -
        d3.select('.value-line').node().getBBox().width
    );

  // GAUGE VALUE LABEL
  if (
    props.value_label_type === 'value' ||
    props.value_label_type === 'both' ||
    props.value_label_type === 'dboth'
  ) {
    var label_pointer = svg
      .append('text')
    ...
    var label_x =
      d3.select('.value-line').node().getBBox().x +
      d3.select('.value-line').node().getBBox().width -
      d3.select('.value-label').node().getBBox().width;
    label_x = label_x < props.w * -0.5 ? props.w * -0.5 : label_x;
    label_pointer
      .attr('x', label_x)
      .attr(
        'y',
        d3.select('.horizontal-fill').node().getBBox().y +
          d3.select('.value-line').node().getBBox().height +
          (d3.select('.value-label').node().getBBox().height * 3) / 4
      );
  }
  if (props.value_label_type === 'label' || props.value_label_type === 'both') {
    var label_pointer = svg
      ...
      .style(
        'font-size',
        `${(props.value_label_font * 3) / 4}${props.limiting_aspect}`
      );
    var label_x =
      d3.select('.value-line').node().getBBox().x +
      d3.select('.value-line').node().getBBox().width -
      d3.select('.value-label-label').node().getBBox().width;
    label_x = label_x < props.w * -0.5 ? props.w * -0.5 : label_x;
    label_pointer
      .attr('x', label_x)
      .attr(
        ...
        d3.select('.horizontal-fill').node().getBBox().y +
          d3.select('.value-line').node().getBBox().height +
          (props.value_label_type === 'label'
            ? d3.select('.value-line').node().getBBox().height / 3
            : d3.select('.value-label').node().getBBox().height * 1.3)
      );
  }
  ...
  if (props.target_source !== 'off') {
    ...
    if (props.target_label_type !== 'none') {
      svg
        ...
        .attr(
          'x1',
          d3.select('.target-fill').node().getBBox().x +
            d3.select('.target-fill').node().getBBox().width
        )
        ...
        .attr(
          'y2',
          d3.select('.target-fill').node().getBBox().y +
      ...
      svg
        ...
        .style(
          'font-size',
          `${props.target_label_font}${props.limiting_aspect}`
        )
        .style('font-family', 'Arial, Helvetica, sans-serif')
        .attr(
          'x',
        ...
        .attr(
          'y',
          d3.select('.target-fill').node().getBBox().y -
            d3.select('.target-label').node().getBBox().height / 2
        );
    }
  }
