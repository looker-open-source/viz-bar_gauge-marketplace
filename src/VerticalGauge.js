...
const drawVertical = props => {
  ...
  svg
    ...
    .attr(
      'width',
      d3.select('.vertical-gauge').attr('width') * (props.arm / 100 + 1)
    )
  ...
  svg
    ...
    .attr(
      'y',
      d3.select('.vertical-gauge').node().getBBox().y +
        d3.select('.vertical-gauge').node().getBBox().height -
        d3.select('.bottom-arm').node().getBBox().height
    );
  ...
  svg
    ...
    .attr(
      'x',
      0 -
        d3.select('.vertical-gauge').node().getBBox().width / 2 -
  ...
  svg
    ...
    .attr(
      ...
      0 -
        d3.select('.vertical-gauge').node().getBBox().width / 2 -
        d3.select('.min-label').node().getBBox().width
    )
  ...
  svg
    ...
    .attr(
      ...
      `${
        proportion *
        (d3.select('.vertical-gauge').node().getBBox().height -
          d3.select('.top-arm').node().getBBox().height * 2)
      }`
    )
    ...
    .attr(
      ...
      d3.select('.vertical-gauge').node().getBBox().y +
        d3.select('.vertical-gauge').node().getBBox().height -
        d3.select('.vertical-fill').node().getBBox().height -
        d3.select('.bottom-arm').node().getBBox().height
    );
  ...
  svg
    ...
    .attr(
      'x',
      0 + d3.select('.vertical-gauge').attr('width') / 2 - props.spinner / 2
    )
  ...
  if (
    ...
    svg
      ...
      .style(
        'font-size',
        `${props.value_label_font * 0.75}${props.limiting_aspect}`
      )
      .attr(
        'x',
  ...
  if (props.value_label_type === 'label' || props.value_label_type === 'both') {
    svg
      ...
      .style(
        'font-size',
        `${props.value_label_font * 0.45}${props.limiting_aspect}`
      )
      .attr(
        'x',
        d3.select('.value-line').node().getBBox().x -
      ...
      .attr(
        'y',
        d3.select('.value-line').node().getBBox().y +
          (props.value_label_type === 'label'
            ? 0
            : d3.select('.value-label').node().getBBox().height)
      );
  }
  ...
  if (props.target_source !== 'off') {
    ...
    // GAUGE TARGET LABEL
    if (
      props.target_label_type === 'value' ||
      props.target_label_type === 'both' ||
      props.target_label_type === 'dboth'
    ) {
      svg
        .append('text')
    ...
    if (
      ...
      svg
        ...
        .style(
          'font-size',
          `${props.target_label_font * (3 / 4)}${props.limiting_aspect}`
        )
        .attr('x', 0 + (d3.select('.vertical-gauge').attr('width') * 3) / 4)
        .attr(
          'y',
          d3.select('.target-line').node().getBBox().y +
            (props.target_label_type === 'label'
              ? 0
              : d3.select('.target-label').node().getBBox().height)
        );
    }
    if (
      props.target_label_type === 'dim' ||
      props.target_label_type === 'dboth'
    ) {
      svg
        .append('text')
